...
void AlocaTabelaPalavras ( char *ficheiro, st_texto *t)
{
  ...
  (*t).n_total_palavras = 0; //t-> é igual a (*t). 
  (*t).n_dist_palavras = 0;
  f = AbreFicheiro ( ficheiro, "r" );
  while ( ( palavra = LePalavra ( f ) ) != NULL ) {
    (*t).n_total_palavras++;
    l = strlen ( palavra ); //len
    if ( l > n_max_caracteres ) //len 
      n_max_caracteres = l; //len melhor que l 
  }
  fclose ( f );
...
void EscreveFicheiro ( char *ficheiro, st_texto *t )
{
  ...
  nome = (char *)malloc(sizeof(char)*(strlen(ficheiro)+strlen(".palavras")+1))/* -- Insert code for memory allocation,  remember to add space for the dot (.), the filename extension and the string termination --*/ ;
  if ( nome == NULL ) {
    fprintf ( stderr, "ERROr: not enough memory available!\n" );
    exit ( 5 ); 
  }
